...
BASE_DIR = os.path.join(WORK_DIR, "batch_datasets")
...
def has_baidu_link(row_data):
    """检查行数据中是否已有百度网盘链接（检查所有列）"""
    for col, val in row_data.items():
        if val and "pan.baidu.com" in val:
            return True
    return False


def get_roboflow_url(row_data):
    """从行数据中提取 Roboflow URL"""
    for col, val in row_data.items():
...
def download_roboflow_dataset(workspace, project, version_num=None, output_dir=None):
    ...
    # 使用 curl 下载（Roboflow 需要 API key）
...
def compress_dataset(source_dir, zip_path):
    """压缩数据集为 ZIP"""
    logger.info(f"  压缩: {source_dir} -> {zip_path}")
...
def process_one_dataset(row_data, access_token):
    """处理单个数据集：下载 -> 压缩 -> 上传 -> 分享 -> 更新表格"""
    row = row_data["row"]
    title = row_data.get("title", "")
    roboflow_url = row_data.get("roboflow_url", "")
    
    logger.info(f"\n{'='*60}")
    logger.info(f"处理: 行{row} - {title[:50]}")
    logger.info(f"Roboflow: {roboflow_url[:80]}")
    
    if not roboflow_url:
        logger.error("  无 Roboflow URL，跳过")
        return False
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="批量处理腾讯文档中没有网盘地址的数据集")
    ...
    logger.info(f"读取腾讯文档: {FILE_ID} / {SHEET_ID}")
    
    # 读取整个表格
    rows = read_sheet_all(TENCENT_TOKEN, FILE_ID, SHEET_ID,
                          start_row=2, end_row=300, start_col=1, end_col=12)
    
    # 找出没有网盘地址且有 Roboflow URL 的数据集
    to_process = []
    for row in sorted(rows.keys()):
        ...
        # 检查是否有 Roboflow URL
        roboflow_url = get_roboflow_url(d)
        if not roboflow_url:
            continue
    ...
    logger.info(f"共找到 {len(to_process)} 个需要处理的数据集")
    
    if args.row:
        to_process = [r for r in to_process if r["row"] == args.row]
    if args.limit:
    ...
    if args.dry_run:
        ...
        for item in to_process:
            logger.info(f"  行{item['row']:3d}: {item['title'][:50]}")
        return
